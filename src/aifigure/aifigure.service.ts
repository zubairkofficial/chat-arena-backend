...
export class AIFigureService extends BaseService {
  constructor(private readonly aiFigureRepository: AIFigureRepository, dataSource: DataSource) {
    super(dataSource)
  }

  // Create a new AIFigure
  async createAIFigure(input: AIFigureDtos.CreateAIFigureDto): Promise<AIFigure> {
  ...
  async updateAIFigure(id: string, input: AIFigureDtos.UpdateAIFigureDto): Promise<AIFigure> {
    ...
    try {
      ...
      throw new BadRequestException(`Error occurred while updating AIFigure with ID ${id}.`);
    }
  }
