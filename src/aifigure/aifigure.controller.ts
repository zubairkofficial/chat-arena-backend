...
export class AIFigureController {
  ...
  @Post()
  async createAIFigure(@Body() createAIFigureDto: AIFigureDtos.CreateAIFigureDto): Promise<AIFigure> {
    try {
      return await this.aiFigureService.createAIFigure(createAIFigureDto);
    } catch (error) {
      handleServiceError(error, HttpStatus.BAD_REQUEST, 'Failed to create AI figure');
    }
  }
  ...
  async getAllAIFigures(): Promise<AIFigure[]> {
    try {
      ...
      handleServiceError(error, HttpStatus.INTERNAL_SERVER_ERROR, 'Failed to retrieve AI figures');
    }
  }
  ...
  async updateAIFigure(
    ...
    try {
      ...
      handleServiceError(error, HttpStatus.BAD_REQUEST, 'Failed to update AI figure');
    }
  }
  ...
  async deleteAIFigure(@Param('id') id: string): Promise<{ message: string }> {
    try {
      ...
      handleServiceError(error, HttpStatus.INTERNAL_SERVER_ERROR, 'Failed to delete AI figure');
    }
  }
}
