...
export class ArenaTypeController {
  ...
  @Post('create')
  async createArenaType(@Body() createArenaTypeDto: ArenaTypeDto.CreateArenaTypeDto | ArenaTypeDto.CreateArenaTypeDto[]) {
    try {
      if (Array.isArray(createArenaTypeDto)) {
        // Handle multiple creation
        return await this.arenaTypeService.createManyArenaTypes(createArenaTypeDto);
      }
      // Handle single creation
      return await this.arenaTypeService.createArenaType(createArenaTypeDto);
    } catch (error) {
      handleServiceError(error, HttpStatus.BAD_REQUEST, 'Failed to create arena type(s)');
    }
  }
  ...
  async getAllArenaTypes() {
    try {
      ...
      handleServiceError(error, HttpStatus.INTERNAL_SERVER_ERROR, 'Failed to retrieve arena types');
    }
  }
  ...
  async updateArenaType(
    ...
    try {
      return await this.arenaTypeService.updateArenaType(id, updateArenaTypeDto);
    } catch (error) {
      handleServiceError(error, HttpStatus.BAD_REQUEST, 'Failed to update arena type');
    }
  }
  ...
  async deleteArenaType(@Param('id') id: string) {
    try {
      ...
      handleServiceError(error, HttpStatus.INTERNAL_SERVER_ERROR, 'Failed to delete arena type');
    }
  }
}
