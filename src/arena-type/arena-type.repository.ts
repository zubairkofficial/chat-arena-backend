...
export class ArenaTypeRepository extends Repository<ArenaType> {
  ...
  async createArenaType(createArenaTypeDto: Partial<ArenaType>): Promise<ArenaType> {
  ...
  async updateArenaType(id: string, updateArenaTypeDto: Partial<ArenaType>): Promise<ArenaType> {
  ...
  async deleteArenaType(id: string): Promise<void> {
    ...
    try {
      ...
      throw new BadRequestException(`Failed to delete ArenaType with ID ${id}.`);
    }
  }
}
